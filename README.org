...
+ Sddm is installed but we don't enable it, because most distros already have a login manager. We should let base Arch installers know that sddm is installed and needs to be enabled.
