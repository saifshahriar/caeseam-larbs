...
#+begin_src bash
choosewm() { \
    whiptail --title "CHOOSE YOUR WINDOW MANAGER(S)" --msgbox "Choose at least one window manager to install. The choices are: Xmonad, Awesome, and Qtile. If unsure, Xmonad is the recommended choice, or install all three options and try out each of them." 16 60
}
