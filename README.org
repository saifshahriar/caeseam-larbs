...
- [[#contributing-to-dtos][Contributing to DTOS]]
  ...
  - [[#test-you-changes-before-pushing][Test you changes before pushing]]
...
You will be asked several =yes/no= questions.  During package installation, pacman will ask you if you want to replace libxft with libxft-bgra.  You must answer YES (y) to this question, or some of the software that will get installed may not work correctly.  During the Doom Emacs part of the installation script, you will be asked two =yes/no= questions.  Answer YES (y) to both questions.
...
** Edit README.org
I understand that non-Emacs users may be confused with the way literate programming works, but the actual DTOS script is README.org which gets converted to dtos.sh.  So the file you should edit is README.org and not dtos.sh.  Through the magic of Org Mode in Emacs, README.org gets converted to dtos.sh.  If you edit dtos.sh and make a merge request, then I have to manually edit README.org to make it conform to your proposed changes.
** Test you changes before pushing
