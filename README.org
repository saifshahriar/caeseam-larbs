...
chaoticaur() {\
    ...
    sudo pacman-key --recv-key 3056513887B78AEB --keyserver keyserver.ubuntu.com && echo "Receiving key for chaotic AUR."
    sudo pacman-key --lsign-key 3056513887B78AEB && echo "Signing key for chaotic AUR."
    yes | sudo pacman -U 'https://cdn-mirror.chaotic.cx/chaotic-aur/chaotic-keyring.pkg.tar.zst' && echo "Installing chaotic AUR keyring."
    yes | sudo pacman -U 'https://cdn-mirror.chaotic.cx/chaotic-aur/chaotic-mirrorlist.pkg.tar.zst' && echo "Installing chaotic AUR mirrorlist."
}
...
addrepo() { \
    echo "###########################################################"
    echo "## Adding the Chaotic AUR repository to /etc/pacman.conf ##"
    echo "###########################################################"
...
* Installing paru
Let's go ahead and install the paru AUR helper because some of the software that the user could choose to install is only found in the AUR.

#+begin_src bash
sudo pacman --noconfirm --needed -Sy chaotic-aur/paru || error "Error installing dtos-core-repo/paru-bin."
...
#+begin_src bash
choosewm() { \
    whiptail --title "CHOOSE YOUR WINDOW MANAGER(S)" --msgbox "Choose at least one window manager to install. The choices are: Qtile, Awesome, Bspwm, Dwm and Xmonad. If unsure, Qtile is the recommended choice, or install all five options and try out each of them." 16 60
}
...
installqtile && sudo pacman --needed --ask 4 -Sy qtile qtile-extras dtos-qtile python-psutil || echo "User chose not to install Qtile"
