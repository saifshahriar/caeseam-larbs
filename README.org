...
#+begin_src bash
sudo pacman --noconfirm --needed -Sy dtos-core-repo/paru-bin || error "Error syncing the repos."
...
#+begin_src bash
choosewm() { \
    whiptail --title "CHOOSE YOUR WINDOW MANAGER(S)" --msgbox "Choose at least one window manager to install. The choices are: Xmonad, Awwesome, and Qtile. If unsure, Xmonad is the recommended choice, or install all three options and try out each of them." 16 60
}
...
installxmonad && sudo pacman -Sy xmonad xmonad-contrib dtos-xmonad xmobar dtos-xmobar polybar dtos-polybar || echo "Not installing XMonad"

installawesome && sudo pacman -Sy awesome dtos-awesome || echo "Not installing Awesome"

installqtile && sudo pacman -Sy qtile dtos-qtile python-psutil || echo "Not installing Qtile"
