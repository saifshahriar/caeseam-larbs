...
- [[#installing-dtos][Installing DTOS]]
  ...
  - [[#you-must-answer-some-yesno-questions][You must answer some YES/NO questions]]
...
To install DTOS, you need to clone this repository and run the dtos.sh script.
#+begin_example
...
** You must answer some YES/NO questions
You will be asked several =yes/no= questions.  During package installation, pacman will ask you if you want to replace libxft with libxft-bgra.  You must answer YES (y) to this question, or some of the software that will get installed may not work correctly.  During the Doom Emacs part of the installation script, you will be asked two =yes/no= questions.  The first question is about creating an envar file, and the second question is about installing all-the-icons fonts.  Answer YES (y) to both questions.

** Potential bugs
*** Permission errors
If you get an error when trying to execute the dtos.sh, make sure that dtos.sh is executable and owned by your user.
*** Slow download times
...
** Edit README.org
I understand that non-Emacs users may be confused with the way literate programming works, but the actual DTOS script is README.org which gets converted to dtos.sh.  So the file you should edit is README.org and not dtos.sh.  Through the magic of Org Mode in Emacs, README.org gets converted to dtos.sh.  If you edit dtos.sh (and not README.org) and make a merge request, then I have to manually edit README.org to make it conform to your proposed changes.  For a big merge, this is a lot work and could lead to mistakes being made in copying/pasting your changes.
