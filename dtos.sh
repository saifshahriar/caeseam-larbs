...
welcome() { \
    ...
    --msgbox "\Z4This is a script that will install what I sarcastically call DTOS (DT's operating system). \
                It's really just an installation script for those that want to try out my XMonad desktop. \
                We will add DTOS repos to Pacman and install the XMonad tiling window manager, \
                the Xmobar panel, the Alacritty terminal, the Fish shell, Doom Emacs \
                and many other essential programs needed to make my dotfiles work correctly. \
...
lastchance() { \
    dialog --colors --title "\Z7\ZbInstalling DTOS!" \
    --msgbox "\Z4WARNING! The DTOS installation script is currently in public beta testing. \
    ...
    --no-label "Exit" \
    --yesno "\Z4Shall we begin installing DTOS?" 8 60 || { clear; exit 1; }
}
